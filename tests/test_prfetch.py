...
def test_repo_exists_returns_info_when_found():
    def runner(args):
        assert args[:2] == ["gh", "api"]
        return json.dumps({"full_name": "o/r", "description": "desc"})

    info = repo_exists("o", "r", runner=runner)
    assert info == {"full_name": "o/r", "description": "desc"}


def test_repo_exists_returns_none_on_404():
    def runner(args):
        raise subprocess.CalledProcessError(1, args)

    assert repo_exists("o", "r", runner=runner) is None


def test_repo_exists_normalizes_null_description():
    info = repo_exists(
        "o", "r", runner=lambda a: json.dumps({"full_name": "o/r", "description": None})
    )
...
def test_search_repos_parses_results():
    ...
    assert (
        res[0]["full_name"] == "facebook/react"
        and res[0]["description"] == "A JS library"
    )
...
def test_classify_gh_error_friendly_for_invalid_search_query():
    ...
    msg = _classify_gh_error(
        'Invalid search query "user:x". ... cannot be searched ...'
    )
...
def test_fetch_pr_parses_issue_from_body_closing_keywords():
    def runner(args):
        if args[:3] == ["gh", "pr", "view"]:
            ...
            return json.dumps(
                {"title": "fix login", "body": "登录超时。Closes #5, also fixes #7"}
            )
        if args[:3] == ["gh", "pr", "diff"]:
            return "diff"
        raise AssertionError(args)
...
def test_fetch_pr_classifies_auth_error():
    def boom(args):
        raise subprocess.CalledProcessError(
            1, args, stderr="gh: not logged in to any hosts"
        )
...
def test_fetch_local_range_and_title():
    data = fetch_local(
        diff_range="main...HEAD", title="我的改动", runner=lambda args: "+y"
    )
