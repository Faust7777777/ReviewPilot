...
| 配置体系 | 🟡 部分 | 分阶段模型 + 每请求 timeout(60s,llm.py);缺 profile / 预算 / 重试 / provider fallback |
| 可观测 / trace | 🟡 部分 | Loop 取证 trace + 护栏丢弃原因已在 briefing 展示(#41/#46);TUI 会话落盘;结构化 run trace JSONL 落盘(`runlog.py`) + **已可读**(`reviewpilot runs`,本轮 #52);**token 用量与 SQLite 仍待** |
| Eval / 回归 | ✅ 方法学到位(实测待 key) | 小样本 FP/FN + **确定性 A/B 对照**(`evaluate_pair`:一次 LLM 双结果,本轮 #52);跨文件样本走 review_loop(#47);loop vs chunked 实测对照待带 key 跑 |
...
| ④ 部分 | **run trace 持久化**:JSONL 落盘 + `reviewpilot runs` 可读(#50/#52);token 用量 + SQLite 仍待 | 🟡 |
| ⑤ | 扩 eval:review_loop + 跨文件样本 + 确定性 A/B 对照(`evaluate_pair`) | ✅ #47/#52(机制+样本;实测待 key) |
| ⑥ | 配置体系:重试 / 预算 / provider fallback(timeout 已做) | ⬜ |
| ⑦ | 跨 PR 记忆:团队规则/历史误报/已驳回结论,作为可检索证据注入 | ⬜ |

## 5. 已知短板(诚实记录)

- 已修:关联 issue 填充(#30/#34)、diffnorm rename/删除/binary 取名(#32)、护栏证据校验(#42)、根 AGENTS.md + CI 回归门(#43)、评审员系统提示外置(#44)、仓库搜索广搜修 422(#45)、护栏失败读取漏洞 + 丢弃可观测(#46)、eval 可走 review_loop + 跨文件样本(#47)、仓库发现 ReAct loop(#49)、run trace JSONL 落盘(#50)、README+demo(#51)、eval 确定性 A/B + runs 读子命令 + LLM timeout(#52)。
- 仍在:run trace 已落 JSONL 且可读,但 **token 用量未记、SQLite 聚合查询未做**;eval 的"loop vs chunked"实测对照待带 key 跑(机制与样本已具备);`chat` 追问仍只带 diff+历史,未接 Review Loop 的按需取证;配置体系(重试/预算/fallback)与进程级沙箱未做。
