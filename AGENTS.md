...
(给"跑在 ReviewPilot 之上的 LLM 评审员"的系统提示**不在这里**——当前在
`review_loop.py` / `analyzer.py` 的字符串里,计划抽成 `docs/REVIEWER_GUIDE.md`。)
...
- `prfetch.py` — 取数据:gh 拉 PR、本地 git diff、`search_repos` 搜真实仓库。
- `resolve.py` — 把一行输入解析成目标(确定性优先,模糊才用 LLM + 用户确认)。
