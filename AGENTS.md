...
核心求解面:评审是一个**受限只读 ReAct 工具循环**——LLM 按需调 `read_file`/`search`/`file_type`/`hex_preview` 取证再出 finding。
...
- `review_loop.py` — **ReAct 评审循环(主求解面)**;`workspace.py` 提供只读 `read_file`/`search`/`file_type`/`hex_preview`(路径穿越防护、浅 clone)。
